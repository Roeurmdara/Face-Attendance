...
export function LiveAttendanceForm() {
    ...
    const loadStudents = async () => {
      try {
        ...
        if (studentsData.length > 0 && withEncodings.length === 0) {
          setError("No students have face encodings. Please re-register students with face capture.");
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const recognitionInterval = setInterval(async () => {
      ...
      try {
        ...
        if (!imageData) { setRecognitionInProgress(false); return; }

        const studentsToCheck = students.filter(
          (s) => s.faceEncodings && Array.isArray(s.faceEncodings) && s.faceEncodings.length > 0
        );
        if (studentsToCheck.length === 0) { setRecognitionInProgress(false); return; }

        for (const student of studentsToCheck) {
          const alreadyMarked = markedAttendance.some((att) => att.studentId === student.id);
          ...
          if (result.recognized && result.confidence && result.confidence > 0.6) {
            try {
              const response = await fetch("/api/attendance", {
                ...
                body: JSON.stringify({ studentId: student.id, confidence: result.confidence }),
              });
              if (response.ok) {
              ...
              setRecognitionError(err instanceof Error ? err.message : "Failed to record attendance");
            }
  ...
  const handleStartAttendance = async () => {
    ...
    if (students.length === 0) { setError("No registered students found"); return; }
    if (studentsWithEncodings === 0) { setError("No students have face encodings saved. Please re-register students."); return; }
    await startCamera();
    setIsRunning(true);
  };
      ...
      <div className="mx-auto max-w-3xl space-y-5">
        ...
        {!isLoading && students.length > 0 && (
          <div className="rounded-xl bg-white border border-slate-100 px-5 py-3 flex items-center gap-3 text-sm">
            ...
            <span className={studentsWithEncodings > 0 ? "text-green-600" : "text-red-500"}>
        ...
        {!isLoading && (
          <div className="space-y-4">
              ...
              <Alert className="border-green-200 bg-green-50 rounded-xl">
                ...
                <AlertDescription>
                  <span className="text-sm font-medium text-green-900">
                    {lastRecognizedStudent.name}
                  </span>
                  <span className="text-sm text-green-700 ml-2">
                    {lastRecognizedStudent.rollNumber} · {lastRecognizedStudent.time} · {getConfidencePercentage(lastRecognizedStudent.confidence)}% confidence
                  </span>
                </AlertDescription>
              </Alert>
            )}
            ...
            {recognitionError && (
              <Alert className="border-red-200 bg-red-50 rounded-xl">
                <AlertCircle className="h-4 w-4 text-red-500" />
                <AlertDescription className="text-red-700 text-sm">{recognitionError}</AlertDescription>
              </Alert>
            )}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-mono text-xs text-slate-400">{att.rollNumber}</p>
                    </div>
                  </div>
